...
import type { GrammarContrastCluster } from '../src/models/grammar.model';
...
describe('compileContrasts', () => {
    ...
    const cluster = (over: Partial<GrammarContrastCluster> = {}): GrammarContrastCluster => ({
        id: 'reason-core',
        label: 'から / ので',
    ...
    it('applies the sanitiser to label, situation, and guidance', () => {
        ...
        const u = index.causality.clusters[0].units[0];
        expect(index.causality.clusters[0].label).toBe('から / ので');
        expect(u.situation).toBe('APOLOGISING.');
        expect(u.guidance).toBe('USE ので.');
    });

    it('throws on an unknown family', () => {
        expect(() => compileContrasts({ nope: { clusters: [cluster()] } }, familyMembers, axisOf))
            .toThrow(/unknown family "nope"/);
    });

    it('throws when a focus/vs id is not a member of the family', () => {
        expect(() => compileContrasts(
            { causality: { clusters: [cluster({ units: [{ focus: 'n4-110', vs: ['n5-999'], situation: 's', guidance: 'g' }] })] } },
            familyMembers,
            axisOf,
    ...
    it("throws when a lesson touches a 'variant'-axis point", () => {
        expect(() => compileContrasts(
            { 'regardless-a-or-b': { clusters: [cluster({ memberIds: ['n1-100', 'n1-101'], units: [{ focus: 'n1-100', vs: ['n1-101'], situation: 's', guidance: 'g' }] })] } },
            familyMembers,
            axisOf,
    ...
    it('throws when focus lists itself in vs', () => {
        expect(() => compileContrasts(
            { causality: { clusters: [cluster({ units: [{ focus: 'n4-110', vs: ['n4-110'], situation: 's', guidance: 'g' }] })] } },
            familyMembers,
            axisOf,
    ...
    it('throws on an empty vs list', () => {
        expect(() => compileContrasts(
            { causality: { clusters: [cluster({ units: [{ focus: 'n4-110', vs: [], situation: 's', guidance: 'g' }] })] } },
            familyMembers,
            axisOf,
    ...
    it('throws when situation or guidance is blank', () => {
        expect(() => compileContrasts(
            { causality: { clusters: [cluster({ units: [{ focus: 'n4-110', vs: ['n5-073'], situation: '  ', guidance: 'g' }] })] } },
            familyMembers,
            axisOf,
